...
document.addEventListener("DOMContentLoaded", () => {
    ...
    */
    const EXAM_DATES = {
        JEE: { 2026: new Date("2026-01-21T00:00:00"), 2027: new Date("2027-01-21T00:00:00"), 2028: new Date("2028-01-21T00:00:00") },
...
*/
        ...
        analytics: {
            ...
            render: (cardElement, cardData) => {
                ...
                const getChartData = (days) => {
                    ...
                    return { labels, dataPoints, totalSecondsRange };
                };
                ...
                const renderChart = (range) => {
                    const { labels, dataPoints, totalSecondsRange } = getChartData(range);
                        ...
                        options: {
                            ...
                            scales: {
                                y: {
                                    ...
                                    ticks: { color: textColor, callback: (val) => (val % 1 === 0 ? val : val.toFixed(1)) + 'h' }
                                },
                                x: {
    ...
    */
    function renderDashboard() {
        ...
        if (!Array.isArray(appState.layout)) {
            ...
            appState.layout = [
                "countdown", "graph", "default-todo", "default-study-logger",
                "default-note", "default-line-graph", "tests", "quote", "time",
            ];
            layoutChanged = true;
        }
    ...
    */
    ...
    domElements.buttons.customize.forEach(btn => btn.addEventListener("click", () =>
    ...
    */
    ...
    function ensureRequiredCards() {
        ...
        if (!appState.customCards.some(c => c.type === 'pomodoro')) {
            const newCard = { id: `custom-${Date.now()}-pom`, type: 'pomodoro', title: 'Pomodoro', content: [] };
            appState.customCards.push(newCard);
            appState.layout.push(newCard.id);
            // Init default state
            appState.pomodoroState[newCard.id] = { mode: "pomodoro", time: 25 * 60, isRunning: false, durations: { pomodoro: 25, shortBreak: 5, longBreak: 15 } };
            added = true;
        }

        // Check Time Logger
        if (!appState.customCards.some(c => c.type === 'time-logger')) {
            const newCard = { id: `custom-${Date.now()}-log`, type: 'time-logger', title: 'Study Logger', content: [] };
            appState.customCards.push(newCard);
            appState.layout.push(newCard.id);
            // Init default state
            appState.timeLoggerState[newCard.id] = { isRunning: false, accumulatedTime: 0, currentSubject: "Physics" };
            added = true;
        }
    ...
    function startFocusLoop() {
        ...
        focusInterval = setInterval(() => {
            const now = new Date();
            if (clockDisplay) clockDisplay.textContent = now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false });
            if (dateDisplay) dateDisplay.textContent = now.toLocaleDateString([], { weekday: 'long', month: 'long', day: 'numeric' });
    ...
    function renderFocusTasks() {
        ...
        if (!todoCard) {
            todoCard = { id: `custom-${Date.now()}-todo`, type: 'todo', title: 'Tasks', content: [] };
    ...
    if (dashAddTaskInput) {
        dashAddTaskInput.addEventListener('keypress', (e) => {
            if (e.key === 'Enter' && e.target.value.trim()) {
                let todoCard = appState.customCards.find(c => c.type === 'todo');
                if (!todoCard) {
                    todoCard = { id: `custom-${Date.now()}`, type: 'todo', title: 'Tasks', content: [] };
                    appState.customCards.push(todoCard);
                }
                todoCard.content.push({ text: e.target.value.trim(), completed: false });
                appState.save("customCards");
                renderFocusTasks();
                e.target.value = '';
            }
